...
set_property -dict { PACKAGE_PIN BD21 IOSTANDARD LVCMOS18 } [get_ports {pciRstL}]
...
set_property -dict { PACKAGE_PIN AU19  IOSTANDARD LVDS } [get_ports { userClkP }]; # 156.25 MHz
set_property -dict { PACKAGE_PIN AV19  IOSTANDARD LVDS } [get_ports { userClkN }]; # 156.25 MHz
...
set_property -dict { PACKAGE_PIN BE17  IOSTANDARD LVCMOS18 } [get_ports { qsfp0RstL }];
set_property -dict { PACKAGE_PIN BD18  IOSTANDARD LVCMOS18 } [get_ports { qsfp0LpMode }];
set_property -dict { PACKAGE_PIN BE20  IOSTANDARD LVCMOS18 } [get_ports { qsfp0ModPrsL }];
set_property -dict { PACKAGE_PIN BE16  IOSTANDARD LVCMOS18 } [get_ports { qsfp0ModSelL }];

##########################################
# QSFP[1] ports located in the core area #
##########################################

set_property -dict { PACKAGE_PIN BC18  IOSTANDARD LVCMOS18 } [get_ports { qsfp1RstL }];
set_property -dict { PACKAGE_PIN AV22  IOSTANDARD LVCMOS18 } [get_ports { qsfp1LpMode }];
set_property -dict { PACKAGE_PIN BC19  IOSTANDARD LVCMOS18 } [get_ports { qsfp1ModPrsL }];
set_property -dict { PACKAGE_PIN AY20  IOSTANDARD LVCMOS18 } [get_ports { qsfp1ModSelL }];
...
create_generated_clock -name sysClk [get_pins {U_Core/U_AxiPciePhy/U_AxiPcie/inst/pcie4_ip_i/inst/gt_top_i/diablo_gt.diablo_gt_phy_wrapper/phy_clk_i/bufg_gt_userclk/O}]

set_clock_groups -asynchronous -group [get_clocks -include_generated_clocks {userClkP}] \
                               -group [get_clocks {sysClk}] \
                               -group [get_clocks {dnaClk}]
                               
set_false_path -from [get_ports {pciRstL}]
set_false_path -to [get_pins -hier -filter {NAME =~ U_Core/U_AxiPciePhy/U_AxiPcie/inst/*sync_reg[0]/D}] 

set_property HIGH_PRIORITY true [get_nets {U_Core/U_BUFG/O}]
set_property HIGH_PRIORITY true [get_nets {U_Core/U_AxiPciePhy/U_AxiPcie/inst/pcie4_ip_i/inst/gt_top_i/diablo_gt.diablo_gt_phy_wrapper/phy_clk_i/bufg_gt_userclk/O}]

create_clock -period  3.333 -name ddrClkP1  [get_ports {mig1DdrClkP}]
create_generated_clock   -name ddrIntClk01  [get_pins {U_Core/U_Mig1/U_MIG/inst/u_ddr4_infrastructure/gen_mmcme4.u_mmcme_adv_inst/CLKOUT0}]
create_generated_clock   -name ddrIntClk11  [get_pins {U_Core/U_Mig1/U_MIG/inst/u_ddr4_infrastructure/gen_mmcme4.u_mmcme_adv_inst/CLKOUT6}]
